...
Instead of passing an actual value to useState, we can pass in a function, and
it will only call that fn to get the state value when the component is rendered
the first time - this optimisation is called **lazy initialisation**

```javascript
React.useState(someExpensiveComputation()) To this: React.useState(() => someExpensiveComputation())
```

A component gets rerendered when its parent gets rerendered.

You can take a bit of logic in your component fn that you think can be useful
elsewhere, you can put that in another fn and call it from any component that
needs it. - **Custom Hooks**-a hook that uses other hooks
...
adding an object as a dependecy to useEffect would throw an ESLint error and
would run the effect on every rerender because React does a shallow comparision
and the object before is not the same as an object after.React does
===/Object.is

What is serializatio/deserialize a string?
